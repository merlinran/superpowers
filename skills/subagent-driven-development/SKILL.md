...
digraph process {
    ...
    "Generate review package, dispatch task reviewer (./task-reviewer-prompt.md)" -> "Record every review finding in progress ledger";
    ...
    "Exactly one valid review verdict that matches findings?" -> "Spec ✅ and quality approved?" [label="yes"];
    ...
    "Append completion to ledger, mark todo complete" -> "Refresh affected and unsettled tasks; rebuild task todos";
    "Refresh affected and unsettled tasks; rebuild task todos" -> "More tasks remain?";
    "More tasks remain?" -> "Dispatch final code reviewer (../requesting-code-review/code-reviewer.md)" [label="no"];
    "Dispatch final code reviewer (../requesting-code-review/code-reviewer.md)" -> "All pending findings explicitly dispositioned?";
    "All pending findings explicitly dispositioned?" -> "Re-dispatch final reviewer for explicit dispositions" [label="no"];
    "Re-dispatch final reviewer for explicit dispositions" -> "Dispatch final code reviewer (../requesting-code-review/code-reviewer.md)";
    ...
    "Dispatch one final fix subagent; re-review" -> "Dispatch final code reviewer (../requesting-code-review/code-reviewer.md)";
    "Final review clean?" -> "Final review clean: delete this plan's workspace" [label="yes"];
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its
  `Plan:` and `Plan revision:` headers match the current plan, tasks with a
  `Task <N>: complete` line are DONE — do not re-dispatch them; resume at the
  first task without one. A task whose last line is a fix round is mid-loop:
  resume the loop at the next round. A ledger whose `Plan:` header names a
  different plan file — or a stray ledger at the old flat path
  `.superpowers/sdd/progress.md` — is another plan's progress: leave it in
  place and start your own, fresh.
- Create the ledger with its identity header lines, one per line, in order:
  `Plan: <path-from-code-repository-root>` then
